/**
 * Admin Emergency Management Routes — Connected to: EmergencyManagement.jsx
 */
...
router.put('/emergency/:id/resolve', ...auth, async (req, res, next) => {
    try {
        const { data, error } = await supabaseAdmin.from('emergency_alerts').update({ status: 'resolved', resolved_at: new Date() }).eq('id', req.params.id).select().single();
        if (error) throw error;
