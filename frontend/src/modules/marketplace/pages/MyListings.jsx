...
const MyListings = () => {
  ...
  const myListings = state.listings.filter(
    (l) => l.ownerName === "Current Resident",
  );
  const filtered =
    activeTab === "all"
      ? myListings
  ...
  const statusCounts = {
    ...
    draft: myListings.filter((l) => l.status === LISTING_STATUS.DRAFT).length,
    pending: myListings.filter((l) => l.status === LISTING_STATUS.PENDING).length,
    approved: myListings.filter((l) => l.status === LISTING_STATUS.APPROVED).length,
    rejected: myListings.filter((l) => l.status === LISTING_STATUS.REJECTED).length,
    sold: myListings.filter((l) => l.status === LISTING_STATUS.SOLD).length,
    rented: myListings.filter((l) => l.status === LISTING_STATUS.RENTED).length,
  };
  return (
          ...
          <table className="mp-table">
            ...
            <tbody>
              {filtered.map((listing) => (
                <tr key={listing.id}>
                  <td>
                    <strong>{listing.flatNumber}</strong>
                    <br />
                    <span
                      style={{
                        fontSize: 12,
