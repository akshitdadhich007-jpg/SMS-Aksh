...
const MarketplaceAnalytics = () => {
  ...
  const stats = useMemo(() => {
    ...
    const active = state.listings.filter((l) =>
      ["approved", "under_visit"].includes(l.status),
    ).length;
    ...
    const totalViews = state.listings.reduce(
      (sum, l) => sum + (l.views || 0),
      0,
    );
    ...
    <div className="mp-page">
      ...
      <div className="mp-stats">
        ...
        <div className="mp-stat-card">
          ...
          <div className="mp-stat-value" style={{ color: "#22c55e" }}>
            {stats.active}
          </div>
        </div>
        <div className="mp-stat-card">
          <div className="mp-stat-label">Sold</div>
          <div className="mp-stat-value" style={{ color: "#8b5cf6" }}>
            {stats.sold}
          </div>
        </div>
        <div className="mp-stat-card">
          <div className="mp-stat-label">Rented</div>
          <div className="mp-stat-value" style={{ color: "#06b6d4" }}>
            {stats.rented}
          </div>
        </div>
        <div className="mp-stat-card">
          <div className="mp-stat-label">Pending</div>
          <div className="mp-stat-value" style={{ color: "#f59e0b" }}>
            {stats.pending}
          </div>
        </div>
        <div className="mp-stat-card">
          <div className="mp-stat-label">Total Views</div>
          <div className="mp-stat-value">{stats.totalViews}</div>
        </div>
        <div className="mp-stat-card">
          <div className="mp-stat-label">Enquiries</div>
          <div className="mp-stat-value">{stats.totalEnquiries}</div>
        </div>
        <div className="mp-stat-card">
          <div className="mp-stat-label">Visits</div>
          <div className="mp-stat-value">{stats.totalVisits}</div>
        </div>
      </div>
